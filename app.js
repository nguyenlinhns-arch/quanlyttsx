...
const CFG={
  sheetId:'14ceKz69_b4-lq70H_5zanSsiQjC1Og9jpFFrTFr2DYE',
...
function renderFallback(){renderKpis(summaryMetrics(FALLBACK.companies));renderCompanyBlocks(FALLBACK.companies);renderProvinceBlocks(FALLBACK.provinces);$('scope').textContent='Toàn bộ dữ liệu · snapshot chuẩn';$('updated').textContent='Snapshot '+FALLBACK.asOf;$('quality').innerHTML='<div class="quality-empty">Đang dùng snapshot chuẩn. Dữ liệu chất lượng cần kết nối trực tiếp.</div>';['dExpected','dEntered','dScore','dSick','dLeave','dKld','dMissing'].forEach(id=>$(id).textContent='—');$('day').innerHTML='<option>Chưa có dữ liệu trực tiếp</option>';$('day').disabled=true;$('coverage').innerHTML='<div class="notice">Chưa kết nối dữ liệu theo ngày.</div>';$('hotspots').innerHTML='<div class="notice">Chưa kết nối dữ liệu chi tiết.</div>'}
...
async function loadFeed(feedId){
  state.feedId=feedId;
  const compP=loadGviz(CFG.companySheet,CFG.companyRange,parseCompanySummary,30000,feedId);
  const provP=loadGviz(CFG.provinceSheet,CFG.provinceRange,parseProvinceSummary,30000,feedId);
  let raw,coreOnly=false;
  try{raw=await loadGviz(CFG.rawSheet,CFG.rawRange,parseRawTable,30000,feedId)}catch(fullErr){console.warn('Full feed failed, retry core',fullErr);raw=await loadGviz(CFG.rawSheet,CFG.rawCoreRange,parseRawTable,30000,feedId);coreOnly=true}
...
async function refresh(){
  ...
  setSync('','Đang kết nối dữ liệu');
  $('notice').className='notice';$('notice').textContent='Đang nạp và đối soát dữ liệu chuẩn...';
  try{
    const {raw,coreOnly,companies,provinces}=await loadFeed(CFG.sheetId);
    state.rows=raw.rows;state.scopeRows=raw.rows;state.dates=coreOnly?[]:raw.headers.slice(7);state.live=true;state.coreOnly=coreOnly;state.companySummary=companies.length?companies:FALLBACK.companies;state.provinceSummary=provinces.length?provinces:FALLBACK.provinces;state.lastRefresh=Date.now();
    hydrate();state.audit=auditConsistency();apply();setSync('ok',coreOnly?'Dữ liệu trực tiếp · lõi':'Dữ liệu trực tiếp');$('notice').className=state.audit.ok?'notice ok':'notice err';$('notice').textContent=(state.audit.ok?'Dữ liệu đã đối soát. ':'Cảnh báo đối soát. ')+state.audit.message+(coreOnly?' Phần theo dõi ngày chưa tải được.':'');$('updated').textContent='Đồng bộ '+new Date().toLocaleString('vi-VN',{hour12:false});
  }catch(e){
    console.error(e);state.live=false;state.coreOnly=false;state.companySummary=FALLBACK.companies;state.provinceSummary=FALLBACK.provinces;renderFallback();setSync('err','Đang dùng snapshot');$('notice').className='notice err';$('notice').textContent='Không kết nối được dữ liệu trực tiếp: '+e.message+'. Dashboard đang dùng snapshot chuẩn đã đối chiếu.';
...
setInterval(()=>{if(document.visibilityState==='visible')$('refresh').click()},CFG.autoRefreshMs);
document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'&&Date.now()-state.lastRefresh>60000)$('refresh').click()});
window.addEventListener('focus',()=>{if(Date.now()-state.lastRefresh>60000)$('refresh').click()});
